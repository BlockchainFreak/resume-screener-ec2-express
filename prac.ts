...
const loadAndClean = async (filename: string) => {
  const JD_Docs = await (new PDFLoader(filename)).load()
  const jd = JD_Docs.map((doc) => doc.pageContent).join("\n")
    .replace(/\n+/g, "\n")
    .replace(/\s+/g, " ")
    .replace(/[^\x00-\x7F]/g, '')
    .replace(", ", ",")
...
const readtxtfile = async (filename: string) => {
  const jd = readFileSync('jobdescs/moreCVs/Android Software Engineer Actalent FL.txt', 'utf-8');
  return jd;
}
...
export const processjD = async (text: string) => {
  const response = await chain.call({ jd: text }) as { jd: JobDesc };
